...
This work is compatible with {{!I-D.ietf-teas-ietf-network-slices}} which describes network slicing in the context of networks built from IETF technologies. This framework introduces the concept of a Network Resource Partition (NRP), which is a collection of resources identified in the underlay network.
Within the scope of this document, {{!I-D.ietf-teas-ietf-network-slices}} is most suited for the WAN domain (provider network), using one NRP or multiple NRPs.
